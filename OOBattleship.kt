...
fun OOBattleship(navyName: String, Guess: Int): String {
    if (
        navyName == JAVA_NAVY && (Guess % 2) == 1
    ) { // oh, you mean odd! and the current state is Java Navy's turn!!
        return KOTLIN_NAVY
    }
    if (
        navyName == KOTLIN_NAVY && isFibonacci(Guess) == 1
    ) { // fibonacci and current state is Kotlin Navy's turn!!
        return GROOVY_NAVY
    }
